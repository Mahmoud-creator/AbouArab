...
@if($product)
    <div {{ $attributes->merge([ "class" => "swiper-slide text-left cursor-pointer" ]) }}>
        <div class="bg-white hover:ring hover:ring-red-500 transition-all">
            ...
            <img data-src="{{ asset($product->image) }}" class="lazyload w-full object-center object-cover" src="{{ asset($product->image) }}" alt="burger">
            <div class="product-element p-5 space-y-2 h-30">
                ...
                @if(auth()->user() && (!isset($isAdmin) || !$isAdmin))
                    <button data-id="{{ $product->id }}" class="rounded add-to-cart hover:scale-110 bg-red-500 hover:bg-red-400 capitalize px-3 py-1.5 text-white transition-all">Add to cart</button>
                    <button data-id="{{ $product->id }}" class="rounded open-product-modal transform hover:scale-110 shadow-md bg-transparent text-red-500 hover:text-white hover:bg-red-500 capitalize px-3 py-1.5 transition-all">Customize</button>
                @endif
